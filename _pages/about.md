---
permalink: /
title: "Acerca de mí"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

## Coordenadas iniciales

Me casé con el amor de mi vida y tenemos una familia hermosa que incluye **tres hijas y un hijo**. 

Soy **abogado** y me gusta **programar** (sí, en **Python** y en otros lenguajes). Disfruto construyendo sistemas que hacen que el trabajo sea más inteligente, más rápido y más humano.

Llevo **más de 10 años trabajando en el Estado**, donde aprendí que las instituciones cambian cuando alguien se toma en serio los detalles y no le tiene miedo a rediseñar lo que “siempre fue así”.

El **dibujo y la fotografía** forman parte de cómo pienso y trabajo. Son medios de expresión que me ayudan a desbloquear ideas, encontrar conexiones y comunicar mejor lo que importa.

Además, estoy construyendo mi propio **Zettelkasten en Obsidian** para potenciar mi producción académica, ordenar mejor el conocimiento y sostener una práctica intelectual más rigurosa.

También soy **docente universitario** y he participado en el **diseño de cursos y programas académicos**, procurando que hagan sentido en el Siglo XXI.

## Mi lema:💡 Implementemos ideas que cambien el mundo.

Ésta es mi fórmula de trabajo: **Procesos (entender el negocio) + Información (controlar lo que ocurre) + Mejoras (con protipos rápidos y sin excusas)**. Además, tengo un principio que no negocio: **nadie llega lejos solo**. Los proyectos que valen la pena se construyen en equipo, con método, con constancia y con gente buena que no se rinde a la primera fricción.

## Lo que estoy construyendo

Trabajo en tres frentes, conectados entre sí:

### 1) Investigación y gestión del conocimiento  
Hay que pararse sobre hombros de gigantes, sí. Pero también hay que **ordenar lo aprendido**, convertirlo en criterio, y hacerlo útil.

### 2) Innovación para resolver problemas reales  
La innovación consiste en **enfrentar y reducir fricciones**, rediseñar procesos y crear herramientas que mejoren resultados sin sacrificar garantías ni rigor.

### 3) Formación del talento que marcará la diferencia  
Diseño y enseño con un objetivo: formar profesionales que combinen **derecho**, **capacidad estratégica** y **alfabetización tecnológica**. El futuro necesita perfiles híbridos capaces de conectar, no compartimentos estancos.

## Una manera de decirlo

Aprendo del pasado para no improvisar.  
Miro al futuro para imaginar lo que podemos construir.  
Trabajo en el presente para aportar, con intención y con método, a la transición que estamos viviendo.

Creo en las ideas bien ejecutadas. Y considero que, cuando se trabaja en serio, sí es posible marcar la diferencia.

## ¿Te interesan los mismos temas que a mí?

**Conversemos**. Esta página contiene enlaces a mis perfiles académicos y profesionales.